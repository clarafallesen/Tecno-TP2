...
void setup(){
  ...
  for (int i = 0; i < 6; i++){
    ...
    float _x = (i * random(50,100) - 50);
    float _y = (i * height/5 - 50);
    p.inicializar(_x, _y, "hierro");
    mundo.add(p);
    plataformas.add(p);
  }
  ...
  lava = new Plataforma(115,50);
  mundo.add(hielo);
  mundo.add(agua);
  mundo.add(lava);
//float _x = (1.5 * random(0,200) - 50);
//float _y = (3.5 * height/5 - 50);
  hielo.inicializar(4 * width/4 - 50, 4 * height/4 - 50, "hielo");//
  agua.inicializar(width/2,height/2, "agua");//
  lava.inicializar(width/2,height/2, "lava");
